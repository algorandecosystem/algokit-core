...
async fn test_create_then_call_app(
    ...
    let abi_return = result.primary_result.abi_return.expect("Expected ABI return");
...
async fn test_construct_transaction_with_abi_encoding_including_transaction(
    ...
    let abi_return = result.primary_result.abi_return.as_ref().expect("Expected ABI return");
...
async fn test_sign_nested_transactions_in_group_with_different_signers(
    ...
    assert_eq!(
        result.primary_result.abi_return.as_ref().unwrap().return_value,

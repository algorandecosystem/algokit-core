    ...
    if !status.is_client_error() && !status.is_server_error() {
        match content_type {
            ...
            ContentType::MsgPack => {
                ...
                GetTransactionGroupLedgerStateDeltasForRound200Response::decode(&content)
                    .map_err(|e| Error::from(serde_json::Error::custom(format!("Failed to decode msgpack response: {}", e))))
            },
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `GetTransactionGroupLedgerStateDeltasForRound200Response`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `GetTransactionGroupLedgerStateDeltasForRound200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetTransactionGroupLedgerStateDeltasForRoundError> =

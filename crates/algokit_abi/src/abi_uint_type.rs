...
mod tests {
    ...
    fn test_uint_size_validation() {
        ...
        for size in valid_sizes {
            let abi_type = ABIType::ABIUintType(BitSize::new(size).unwrap());
    ...
    fn test_uint256_large_values() {
        let abi_type = ABIType::ABIUintType(BitSize::new(256).unwrap());
    ...
    fn test_uint_decode_wrong_length() {
        let test_cases = vec![
            (ABIType::ABIUintType(BitSize::new(8).unwrap()), vec![0u8, 0]), // 2 bytes for uint8
            (ABIType::ABIUintType(BitSize::new(16).unwrap()), vec![0u8]),   // 1 byte for uint16
            (
                ABIType::ABIUintType(BitSize::new(32).unwrap()),
                vec![0u8, 0, 0],
            ), // 3 bytes for uint32
            (
                ABIType::ABIUintType(BitSize::new(64).unwrap()),
                vec![0u8; 7],
            ), // 7 bytes for uint64
        ];
    ...
    fn test_uint_encoding_deterministic() {
        let test_cases = vec![
            (
                ABIType::ABIUintType(BitSize::new(8).unwrap()),
                ABIValue::Uint(BigUint::from(42u8)),
            ),
            (
                ABIType::ABIUintType(BitSize::new(16).unwrap()),
    ...
    fn test_uint_max_size() {
        let abi_type = ABIType::ABIUintType(BitSize::new(512).unwrap());

...
export interface FieldMetadata {
  ...
  /** Default value from OpenAPI schema, if present. */
...
export class AlgorandSerializer {
  ...
  private static transformObject(value: unknown, meta: ModelMetadata, ctx: TransformContext): unknown {
    ...
    if (ctx.direction === 'encode') {
      ...
      // Apply default values for missing fields before encoding (OpenAPI defaults)
      for (const field of fields) {
        if (src[field.name] === undefined && field.defaultValue !== undefined) {
          ;(src as Record<string, unknown>)[field.name] = field.defaultValue
        }
      }
    ...
    const src = value as Record<string, unknown>
    // Apply default values for missing fields (OpenAPI defaults)
    for (const field of fields) {
      if (src[field.name] === undefined && field.defaultValue !== undefined) {
        ;(src as Record<string, unknown>)[field.name] = field.defaultValue
      }

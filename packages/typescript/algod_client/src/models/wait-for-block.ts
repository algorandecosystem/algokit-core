...
// JSON DTO shape for WaitForBlock with wire keys and JSON-safe primitives
export type WaitForBlockDto = {
  'catchup-time': string
  'last-round': string
  'last-version': string
  'next-version': string
...
// toDto/fromDto
export function toDto(value: WaitForBlock): WaitForBlockDto {
  const out: any = {}
  {
  ...
  return out as WaitForBlockDto
}

export function fromDto(dto: WaitForBlockDto): WaitForBlock {
  const out: any = {}
  {
  ...
  return out as WaitForBlock
}

// Msgpack codecs
export function encodeMsgpack(value: WaitForBlock): Uint8Array {
  const dto = toMsgpackDto(value)
  return encodeMsgPack(dto)
}

export function decodeMsgpack(bytes: Uint8Array): WaitForBlock {
...
// JSON codecs
export function encodeJson(value: WaitForBlock): unknown {
  return toDto(value)
}

export function decodeJson(raw: unknown): WaitForBlock {
  return fromDto(raw as WaitForBlockDto)
}

// Array helpers
export function encodeMsgpackArray(values: WaitForBlock[]): Uint8Array {
  const dto = values.map((v) => toMsgpackDto(v))
  return encodeMsgPack(dto)
}

export function decodeMsgpackArray(bytes: Uint8Array): WaitForBlock[] {
  const raw: any = decodeMsgPack(bytes)
  return (raw as any[]).map((item) => fromMsgpackDto(item))
}

export function encodeJsonArray(values: WaitForBlock[]): unknown {
  return values.map((v) => toDto(v))
}

export function decodeJsonArray(raw: unknown): WaitForBlock[] {
  return (raw as any[]).map((item) => fromDto(item))
}

// Internal: msgpack DTO (wire keys, bytes kept as Uint8Array, signed txn encoded to bytes)
type WaitForBlockMsgpackDto = {
  'catchup-time': bigint
  'last-round': bigint
  'last-version': string
  'next-version': string
...
function toMsgpackDto(value: WaitForBlock): WaitForBlockMsgpackDto {
  const out: any = {}
  {
    const v = (value as any)['catchupTime']
    if (v === undefined) {
  ...
  return out as WaitForBlockMsgpackDto
}

function fromMsgpackDto(dto: WaitForBlockMsgpackDto): WaitForBlock {
  const out: any = {}
  {
    const v = (dto as any)['catchup-time']
    if (v === undefined) {

...
function decodeNestedSignedTransactions(modelKey: string | undefined, value: any, mediaType: Media): any {
  // eslint-disable-line @typescript-eslint/no-explicit-any
  if (!modelKey || value == null || typeof value !== 'object') return value
...
function decodeAtPath(current: any, tokens: string[], idx: number, mediaType: Media): any {
  // eslint-disable-line @typescript-eslint/no-explicit-any
...
function decodeLeaf(value: any, mediaType: Media): any {
  // eslint-disable-line @typescript-eslint/no-explicit-any
  if (value == null) return value
...
function decodeSignedTransactionsAuto(value: any): any {
  // eslint-disable-line @typescript-eslint/no-explicit-any
  if (value == null) return value
  if (value instanceof Uint8Array) return value
  ...
  // Heuristic: object with a 'txn' sub-object that looks like TransactionDto → decode as SignedTransaction
...
function toStxMsgpackReady(value: any): any {
  // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  for (const [k, v] of Object.entries(value)) {
    ...
    if (typeof v === 'string' && shouldTreatAsBytesKey(k)) {
      ...
      out[k] = v.map((item) =>
        typeof item === 'string' && shouldTreatAsBytesKey(k) ? (tryBase64ToBytes(item) ?? item) : toStxMsgpackReady(item),
      )
...
function encodeNestedSignedTransactions(modelKey: string | undefined, value: any): any {
  // eslint-disable-line @typescript-eslint/no-explicit-any
  if (!modelKey || value == null || typeof value !== 'object') return value
  const paths = STX_FIELDS[modelKey]
  if (!paths || paths.length === 0) return value
...
function encodeAtPath(current: any, tokens: string[], idx: number): any {
  // eslint-disable-line @typescript-eslint/no-explicit-any
  if (current == null) return current
  if (idx >= tokens.length) {

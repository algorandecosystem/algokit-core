...
import { IntDecoding, parseJson, stringifyJson } from './json'
import { decodeMsgPack, encodeMsgPack } from './msgpack'
...
export async function request<T>(
  ...
  if (options.body != null) {
    if (options.mediaType?.includes('json')) {
      ...
      body = options.body instanceof Uint8Array ? options.body : encodeMsgPack(options.body)
    } else {
      body = options.body
    }
  }
  ...
  if (!response.ok) {
    ...
    try {
      ...
      if (ct && ct.includes('application/json')) body = parseJson(await response.text(), config.intDecoding ?? IntDecoding.MIXED)
  ...
  // Handle msgpack responses - return raw bytes for model codecs to decode
  ...
  if (contentType.includes('application/json')) {
    ...
    const parsed = parseJson(text, config.intDecoding ?? IntDecoding.MIXED)
    return parsed as T
  }

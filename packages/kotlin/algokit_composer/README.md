# algokit_composer (JVM)

A Kotlin/JVM ("desktop") build of `algokit_composer`, for use in any JVM application
(desktop apps, servers, CLIs, etc.) rather than Android. It's built from the exact
same Rust core and uniffi-generated Kotlin bindings as `packages/android/algokit_composer`
...
`win32-x86-64`), and produces a JAR at `build/libs/algokit_composer-*.jar`.

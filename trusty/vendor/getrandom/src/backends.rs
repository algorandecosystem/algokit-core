// Vendored verbatim from `getrandom` 0.4.1 -- see `../src/lib.rs` for
// provenance/trim notes. Kept as-is (including every non-`custom` arm) so
// this file matches upstream byte-for-byte and stays easy to diff/update;
// only the `custom` arm is actually reachable given `--cfg
// getrandom_backend="custom"` (see `../rules.mk`), everything else is inert

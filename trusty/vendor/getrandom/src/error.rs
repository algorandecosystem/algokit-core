...
// provenance notes. The `#[cfg(feature = "std")] impl std::error::Error`
// block from upstream's `error_std_impls.rs` is dropped along with that
// file: this build never enables the `std` feature (see `../rules.mk`), so
// it would be dead code here regardless.

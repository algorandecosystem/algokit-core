...
- Node.js 20+ - JavaScript runtime (for convert-openapi script)
...
Generate both TypeScript API clients and run format + tests:
...
```bash
# Generate algod TypeScript client only
cargo api generate-ts-algod

# Generate indexer TypeScript client only
cargo api generate-ts-indexer
```

The generated TS clients will be available at:

- `../packages/typescript/algod_client/src/`
- `../packages/typescript/indexer_client/src/`

Root files (package.json, tsconfigs, eslint, README, rolldown) are preserved; only `src/` is overwritten. Existing `__tests__/` are kept intact.
